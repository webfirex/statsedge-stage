      ...
      <Table.Tr key={element.player}>
        ...
        <Table.Td>
            <Box display={"flex"} style={{ gap: "10px", border: "1px solid #1d1d1d", padding: "5px", borderRadius: "5px" }} miw={"80"} maw={"80"} bg={"#071225"}>
              <Text ta={"center"} w={"100%"} fz={"sm"}>{element.odds}</Text>
            </Box>
        </Table.Td>
        <Table.Td>{element.se}</Table.Td>
        <Table.Td p={"5"}>
          <Flex direction={"column"} gap={"5px"}>
            <Text ta={"center"} fz={"xs"}>{element.i1[0]}</Text>
            <Card bg={"#3a3a3a"} p={"xs"} fz={"xs"} ta={"center"}>{element.i1[1]}</Card>
          </Flex>
        </Table.Td>
        <Table.Td p={"5"}>
          <Flex direction={"column"} gap={"5px"}>
            <Text ta={"center"} fz={"xs"}>{element.i2[0]}</Text>
            <Card bg={"#3a3a3a"} p={"xs"} fz={"xs"} ta={"center"}>{element.i2[1]}</Card>
          </Flex>
        </Table.Td>
      </Table.Tr>
    ));
          ...
          <Table striped highlightOnHover  horizontalSpacing="sm" verticalSpacing="sm">
            <Table.Thead>
              <Table.Tr bg={"#101010"} color="#005BBF">
                ...
                <Table.Th color="#005BBF"><Image src={"/logo.png"} alt="logo" w={"25"} /></Table.Th>
                <Table.Th color="#005BBF"><Flex justify={"center"}><Image src={"/icon-3.png"} alt="logo" w={"25"} /></Flex></Table.Th>
                <Table.Th color="#005BBF"><Flex justify={"center"}><Image src={"/icon-4.png"} alt="logo" w={"25"} /></Flex></Table.Th>
              </Table.Tr>
            </Table.Thead>
